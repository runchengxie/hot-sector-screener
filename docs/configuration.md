...
| `adapter` | `chat_completions` | 数据提供方中立（provider-neutral）的 JSON 线协议适配器 |
| `prompt_template` | `default` | 提示词模板，当前仅支持 default |

远端模式不提供任何内建 endpoint、凭据、模型或数据提供方（provider）默认值，启动前必须显式设置：
...
| `LLM_PROVIDER_ID` | 部署侧数据提供方（provider）标识，仅写入本地内部 lineage |
...
远端返回的自由文本在进入候选池前还会执行客户文案泄漏校验：数据提供方（provider）标识、模型标识、
