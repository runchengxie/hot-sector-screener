...
远端分类不带数据提供方（provider）或模型默认值，运行前必须通过私有环境显式设置
...
这里的 `2026-06-19` 只作为数据截止日（EOD，end of day）使用。候选池最早只能供下一交易时段使用，历史运行的 rotation 信号也会严格限制为 `signal_date <= observation_date`。
...
- `candidate_quality.json` / `candidate_outcomes.json` — 生成阶段只写延迟占位（deferred stub），不读取任何未来行情，事后评价由独立研究 owner/流程执行
